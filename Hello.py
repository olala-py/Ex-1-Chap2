...
class SimpleMotor:
    def __init__(
        ...
        tau_max=2.0,     # Maximum torque [Nm]
        Kp=10.0,         # Proportional gain (PD controller)
        Kd=1.0,          # Derivative gain (PD controller)
    ...
    def simulate(self, theta_target):
        ...
        theta     = 1.0   # Góc [rad]
        theta_dot = 0.0   # Vận tốc góc [rad/s]
        ...
        for i in range(self.steps):
            # ── Bộ điều khiển PD ────────────────────────────
            error = theta_target - theta          # Sai lệch vị trí
            tau   = self.Kp * error - self.Kd * theta_dot   # Torque điều khiển

            # Giới hạn torque (saturation)
            tau = np.clip(tau, -self.tau_max, self.tau_max)

            # ── Vật lý động cơ (Newton-Euler) ───────────────
            # J·θ'' = τ - b·θ'
